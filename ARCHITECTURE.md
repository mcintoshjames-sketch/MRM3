...
- **Permission Utilities** (`src/utils/roleUtils.ts`): Single source of truth for role-based permission checks in the frontend. Helper functions like `canManageValidations(user)`, `canManageModels(user)`, `isAdmin(user)`, etc. check `user.capabilities` first (server-provided), falling back to role-based logic. Use these helpers in components to conditionally render UI elements based on user permissions. This pattern ensures permission logic is centralized and consistent across all pages.
...
- **IRP (Independent Review Process)**: Governance mechanism covering high-risk MRSAs. Fields: irp_id, process_name, description, contact_user_id, is_active. Relationships: covered_mrsas (many-to-many via mrsa_irp), reviews (one-to-many), certifications (one-to-many). **IRPReview**: Periodic assessment with review_date, outcome_id (taxonomy), notes, reviewed_by_user_id. **IRPCertification**: MRM sign-off with certification_date, certified_by_user_id, certified_by_email (email of certifier), conclusion_summary.
...
- **Historical Snapshot Behavior**:
  - When a scorecard is first computed, the current config is captured in `config_snapshot` (immutable thereafter)
  - API returns `config_sections` in scorecard response, sourced from snapshot for existing scorecards or current config for new ones
  - Editing ratings on existing scorecards validates against the historical snapshot, not current config
  - This ensures: (a) removed criteria can still be rated on historical scorecards, (b) new criteria don't appear on old scorecards, (c) weight changes don't affect historical computations
- **API Endpoints** (prefix: `/scorecard`):
  - Configuration: `GET /config` - Get current scorecard configuration (sections with nested criteria)
  - Ratings: `GET /validation/{request_id}` (returns `config_sections` from snapshot), `POST /validation/{request_id}`, `PATCH /validation/{request_id}/ratings/{criterion_code}`, `PATCH /validation/{request_id}/overall-narrative`
