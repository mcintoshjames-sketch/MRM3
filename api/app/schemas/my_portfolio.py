...
class PortfolioSummary(BaseModel):
    ...
    total_models: int = Field(..., description="Total number of models in portfolio")
    action_items_count: int = Field(..., description="Number of items requiring action")
    overdue_count: int = Field(..., description="Number of overdue items")
    compliant_percentage: float = Field(..., description="Percentage of models that are compliant")
    models_compliant: int = Field(..., description="Number of compliant models")
...
class ActionItem(BaseModel):
    ...
    type: str = Field(..., description="Type: attestation, recommendation, validation_submission")
    urgency: str = Field(..., description="Urgency: overdue, in_grace_period, due_soon, upcoming")
    ...
    item_code: Optional[str] = Field(None, description="Code/reference (e.g., REC-2025-00042)")
    title: str = Field(..., description="Brief title of the action needed")
    action_description: str = Field(..., description="Description of the action required")
    due_date: Optional[date] = Field(None, description="Due date for the action")
...
class MonitoringAlert(BaseModel):
    ...
    metric_value: Optional[float] = Field(None, description="Numeric value (if quantitative)")
    qualitative_outcome: Optional[str] = Field(None, description="Qualitative outcome label")
...
class CalendarItem(BaseModel):
    ...
    model_name: str = Field(..., description="Name of the related model")
    item_id: int = Field(..., description="ID of the specific item")
    item_code: Optional[str] = Field(None, description="Code/reference if applicable")
...
class PortfolioModel(BaseModel):
    ...
    risk_tier: Optional[str] = Field(None, description="Risk tier label (e.g., 'Tier 1')")
    risk_tier_code: Optional[str] = Field(None, description="Risk tier code")
    approval_status: Optional[str] = Field(None, description="Model approval status label (e.g., 'Approved', 'Expired')")
    approval_status_code: Optional[str] = Field(None, description="Model approval status code (e.g., 'APPROVED', 'EXPIRED')")
    last_validation_date: Optional[date] = Field(None, description="Date of last validation")
    next_submission_due: Optional[date] = Field(None, description="Next submission due date")
    next_validation_due: Optional[date] = Field(None, description="Next validation due date")
    days_until_submission_due: Optional[int] = Field(None, description="Days until next submission due")
    days_until_validation_due: Optional[int] = Field(None, description="Days until next validation due")
    open_recommendations: int = Field(0, description="Count of open recommendations")
